...
static uint8_t l_ShouldRecordAllocations = 0;
static uint8_t l_ShouldPrintAllocations = 0;
static size_t l_CurrentAllocatedByteCount = 0;
static size_t l_MaxAllocatedByteCount = 0;
...
psa_status_t CLI_ImportHmacKey(
    const uint8_t *pKey,
    size_t keyByteCount,
    psa_key_id_t *keyId);

psa_status_t CLI_ImportDerivationKey(
    const uint8_t *pKey,
    size_t keyByteCount,
    psa_key_id_t *keyId);
...
int main(int argc, char **argv) {
    ...
    mbedtls_platform_set_calloc_free(CLI_Calloc, CLI_Free);
    ...
    status =
            CLI_ImportHmacKey(l_AuditLogKey, sizeof(l_AuditLogKey), &auditLogKeyId);
    if (status != PSA_SUCCESS) {
        (void) printf(
    ...
    status = CLI_ImportDerivationKey(
    ...
    (void) printf("HOST Shared Secret = ");
    ...
    (void) printf("Session Output Salt = 0x%016llx\n", sessionOutputSalt);
    ...
    CLI_PrintBytes(ciphertext, ciphertextWrittenByteCount);
    (void) printf("Tag = ");
    CLI_PrintBytes(tag, tagWrittenByteCount);
    ...
    (void) printf("Decrypted Plaintext = '");
    for (size_t i = 0; i < writtenByteCount; i++) {
        if (plaintext[i] == 0) {
            continue;
        }

        (void) printf("%c", plaintext[i]);
    }
    (void) printf("'\n");
...
CLEAN_UP:
    (void) printf("Max Allocated Byte Count = %zu\n", l_MaxAllocatedByteCount);
...
int CLI_InitializeMutex(mbedtls_platform_mutex_t *pMutex) {
    ...
    pMutex->data = CLI_Calloc(1, sizeof(mtx_t));
    if (pMutex->data == NULL) {
        return MBEDTLS_ERR_THREADING_USAGE_ERROR;
    }

    if (mtx_init(pMutex->data, mtx_plain) != thrd_success) {
        CLI_Free(pMutex->data);
        pMutex->data = NULL;
        return MBEDTLS_ERR_THREADING_USAGE_ERROR;
    }
...
void *CLI_Calloc(size_t n, size_t size) {
    size_t requestedByteCount = n * size;
    uint8_t *pBuffer = malloc(sizeof(size_t) + requestedByteCount);
    if (pBuffer == NULL) {
        return NULL;
    }

    size_t *pBlockByteCount = (size_t *) pBuffer;
    *pBlockByteCount = requestedByteCount;

    uint8_t *pBlock = pBuffer + sizeof(size_t);
    memset(pBlock, 0, requestedByteCount);

    if (l_ShouldRecordAllocations != 0) {
        l_CurrentAllocatedByteCount += requestedByteCount;
        if (l_CurrentAllocatedByteCount > l_MaxAllocatedByteCount) {
            l_MaxAllocatedByteCount = l_CurrentAllocatedByteCount;
        }
    }

    if (l_ShouldPrintAllocations != 0) {
        (void) printf(
            "Allocated %zu bytes (n = %zu, size = %zu).\n",
            requestedByteCount,
            n,
            size);
    }
...
void CLI_Free(void *ptr) {
    ...
    uint8_t *pBuffer = (uint8_t *) ptr - sizeof(size_t);
    size_t blockByteCount = *(size_t *) pBuffer;

    free(pBuffer);

    if (l_ShouldRecordAllocations != 0) {
        l_CurrentAllocatedByteCount -= blockByteCount;
    }

    if (l_ShouldPrintAllocations != 0) {
        (void) printf("Freed %zu bytes.\n", blockByteCount);
    }
}
...
void CLI_PrintBytes(const uint8_t *pBytes, const size_t byteCount) {
    ...
    for (size_t i = 0; i < byteCount; i++) {
        (void) printf("%02x", pBytes[i]);
    }
    (void) printf("\n");
}

void CLI_PrintKey(const psa_key_id_t keyId, const char *keyName) {
    uint8_t keyBuffer[32] = {0};
    size_t writtenByteCount = 0;
    psa_status_t status =
            psa_export_key(keyId, keyBuffer, sizeof(keyBuffer), &writtenByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to print the %s Key. Status = %d\n", keyName, status);
        return;
    }

    (void) printf("%s Key = 0x", keyName);
    for (size_t i = 0; i < writtenByteCount; i++) {
        (void) printf("%02x", keyBuffer[i]);
    }
    (void) printf("\n");
}
...
psa_status_t CLI_CreateSessionKeyAndSalt(
    ...
    uint8_t *payload = CLI_Calloc(32 + infoByteCount + 1, sizeof(uint8_t));
    if (payload == NULL) {
...
psa_status_t CLI_Encrypt(
    ...
    uint8_t *pCiphertext,
    const size_t ciphertextByteSize,
    size_t *pCiphertextWrittenByteCount,
    uint8_t *tag,
    const size_t tagByteCount,
    size_t *pTagWrittenByteCount) {
    if (pPlaintext == NULL || pCiphertext == NULL ||
        pCiphertextWrittenByteCount == NULL || tag == NULL ||
        pTagWrittenByteCount == NULL) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    ...
    psa_status_t status = psa_aead_encrypt_setup(
        ...
        PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CCM, 8));
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to set up AEAD encryption. Status = %d\n", status);
        return status;
    }
    ...
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to set the nonce for AEAD encryption. Status = %d\n", status);
        return status;
    }
    ...
    status = psa_aead_set_lengths(
        ...
        plaintextByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to set the plaintext length for AEAD encryption. Status = %d\n", status);
        return status;
    }

    size_t updateWrittenByteCount = 0;
    status = psa_aead_update(
        ...
        pCiphertext,
        ciphertextByteSize,
        &updateWrittenByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to update AEAD encryption. Status = %d\n", status);
        return status;
    }

    size_t finishWrittenByteCount = 0;
    status = psa_aead_finish(
        &operation,
        pCiphertext + updateWrittenByteCount,
        ciphertextByteSize - updateWrittenByteCount,
        &finishWrittenByteCount,
        tag,
        tagByteCount,
        pTagWrittenByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to finish AEAD encryption. Status = %d\n", status);
        return status;
    }

    *pCiphertextWrittenByteCount =
            updateWrittenByteCount + finishWrittenByteCount;
    return status;
}
...
psa_status_t CLI_Decrypt(
    ...
    const uint32_t counter,
    const uint8_t *pCiphertext,
    const size_t ciphertextByteCount,
    const uint8_t *pTag,
    const size_t tagByteCount,
    uint8_t *pPlaintext,
    const size_t plaintextByteSize,
    size_t *pPlaintextWrittenByteCount) {
    if (pCiphertext == NULL || pTag == NULL || pPlaintext == NULL ||
    ...
    psa_status_t status = psa_aead_decrypt_setup(
        ...
        PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CCM, 8));
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to set up AEAD decryption. Status = %d\n", status);
        return status;
    }
    ...
    status = psa_aead_set_nonce(&operation, nonce, sizeof(nonce));
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to set the nonce for AEAD decryption. Status = %d\n", status);
        return status;
    }
    ...
    status = psa_aead_set_lengths(
        ...
        ciphertextByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to set the ciphertext length for AEAD decryption. Status = %d\n", status);
        return status;
    }
    ...
    status = psa_aead_update(
        ...
        &updateWrittenByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to update AEAD decryption. Status = %d\n", status);
        return status;
    }

    size_t verifyWrittenByteCount = 0;
    status = psa_aead_verify(
        &operation,
        pPlaintext + updateWrittenByteCount,
        plaintextByteSize - updateWrittenByteCount,
        &verifyWrittenByteCount,
        pTag,
        tagByteCount);
    if (status != PSA_SUCCESS) {
        (void) printf("Failed to verify AEAD decryption. Status = %d\n", status);
        return status;
    }

    *pPlaintextWrittenByteCount =
            updateWrittenByteCount + verifyWrittenByteCount;
    return status;
}

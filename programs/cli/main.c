...
psa_status_t CLI_ImportHmacKey(const uint8_t* pKey,
                               size_t keyByteCount,
                               psa_key_id_t* keyId);

psa_status_t CLI_ImportDerivationKey(const uint8_t* pKey,
                                     size_t keyByteCount,
                                     psa_key_id_t* keyId);
...
psa_status_t CLI_CreateSessionKeyAndSalt(const uint8_t* pPrivateKey,
...
int main(int argc, char** argv) {
    ...
    mbedtls_threading_set_alt(CLI_InitializeMutex,
                              CLI_DestroyMutex,
                              CLI_LockMutex,
                              CLI_UnlockMutex,
                              NULL,
    ...
    if (status != PSA_SUCCESS) {
        printf("Failed to create the %s Key. Status = %d\n",
               auditLogKeyName,
               status);
        goto CLEAN_UP;
    }
    ...
    status = CLI_ImportDerivationKey(l_MasterDerivationKey,
    ...
    if (status != PSA_SUCCESS) {
        printf("Failed to get the public key for the %s Key. Status = %d\n",
               hostKeyName,
               status);
        goto CLEAN_UP;
    }
    ...
    status = CLI_CreateSessionKeyAndSalt(l_MasterDerivationKey,
                                         sizeof(l_MasterDerivationKey),
                                         sessionInputSalt,
    ...
    status = CLI_Sign(auditLogKeyId,
                      ...
                      &macWrittenByteCount);
    if (status != PSA_SUCCESS) {
        printf("Failed to sign data with the %s Key. Status = %d\n",
               sessionKeyName,
               status);
        goto CLEAN_UP;
    }
    ...
    if (status != PSA_SUCCESS) {
        printf("Failed to encrypt data with the %s Key. Status = %d\n",
               sessionKeyName,
               status);
        goto CLEAN_UP;
    }
    ...
    status = CLI_Decrypt(sessionKeyId,
                         ...
                         &writtenByteCount);
    if (status != PSA_SUCCESS) {
        printf("Failed to decrypt data with the %s Key. Status = %d\n",
               sessionKeyName,
               status);
        goto CLEAN_UP;
    }
...
CLEAN_UP:
    printf("Max Allocated Byte Count = %zu\n", l_MaxAllocatedByteCount);
...
void* CLI_Calloc(size_t n, size_t size) {
    ...
    *pBlockByteCount = requestedByteCount;
    ...
    if (l_ShouldPrintAllocations != 0) {
        printf("Allocated %zu bytes (n = %zu, size = %zu).\n",
               requestedByteCount,
               n,
               size);
    }
...
void CLI_PrintKey(const psa_key_id_t keyId, const char* keyName) {
    ...
    if (status != PSA_SUCCESS) {
        printf("Failed to print the %s Key. Status = %d\n", keyName, status);
        return;
    }
...
psa_status_t CLI_GetPublicKey(const psa_key_id_t privateKeyId,
                              uint8_t* pPublicKey,
...
psa_status_t CLI_CreateSharedSecret(const psa_key_id_t privateKeyId,
                                    const uint8_t* pPeerPublicKey,
...
psa_status_t CLI_CreateSessionKeyAndSalt(const uint8_t* pPrivateKey,
    ...
    status = psa_import_key(
        &keyAttributes, pInputSalt, inputSaltByteCount, &generatedKeyId);
    if (status != PSA_SUCCESS) {
        printf(
    ...
    uint8_t prk[32] = {0};
    size_t writtenByteCount = 0;
    status = psa_mac_compute(generatedKeyId,
    ...
    while (okmByteCount < sizeof(okm)) {
        ...
        status = psa_mac_compute(generatedKeyId,
                                 ...
                                 &writtenByteCount);
        if (status != PSA_SUCCESS) {
            printf("Failed to compute a MAC for the expand phrase. Status = %d",
                   status);
            goto CLEAN_UP;
        }
    ...
    if (status != PSA_SUCCESS) {
        printf("Failed to create a session key from the OKM. Status = %d\n",
               status);
        goto CLEAN_UP;
    }
...
psa_status_t CLI_Sign(const psa_key_id_t keyId,
                      const uint8_t* pInput,
...
psa_status_t CLI_Encrypt(const psa_key_id_t keyId,
                         const uint64_t salt,
...
psa_status_t CLI_Decrypt(const psa_key_id_t keyId,
                         const uint64_t salt,
                         const uint32_t counter,
                         const uint8_t* pAuthCiphertext,
...
psa_status_t CLI_Encrypt2(const psa_key_id_t keyId,
                          const uint64_t salt,
                          const uint32_t counter,
                          const uint8_t* pPlaintext,
                          const size_t plaintextByteCount,
    ...
    psa_status_t status = psa_aead_encrypt_setup(
        &operation, keyId, PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CCM, 8));
    if (status != PSA_SUCCESS) {
        // TODO: Log.
        return status;
    }
...
psa_status_t CLI_Decrypt2(const psa_key_id_t keyId,
                          const uint64_t salt,
                          const uint32_t counter,
